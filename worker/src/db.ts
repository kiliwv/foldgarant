...
export class Db {
  ...
  /** История: только завершённые и возвраты — отменённые не показываем. */
  async userClosedDeals(userId: number, limit = 10): Promise<DealRow[]> {
    const res = await this.d1
      ...
      .bind(userId, userId, userId, COMPLETED, REFUNDED, limit)
      .all<DealRow>();
    return res.results;
  }
  ...
  async ensureBalancesTable(): Promise<void> {
    ...
    try {
      await this.d1
        .prepare("ALTER TABLE users ADD COLUMN is_gold INTEGER NOT NULL DEFAULT 0")
        .run();
    } catch {
      // колонка уже существует
    }
  ...
  async setGold(userId: number, gold: boolean): Promise<boolean> {
    const res = await this.d1
