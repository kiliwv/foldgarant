...
# Telegram Bot API не позволяет менять цвет кнопок, поэтому «цвет» задаём
# эмодзи-индикаторами (так же делает @CryptoBot).
ASSET_COLORS = {"USDT": "🟢", "TON": "🔵", "BTC": "🟠"}
...
def asset_kb(assets: list[str]) -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    for asset in assets:
        emoji = ASSET_COLORS.get(asset, "⚪️")
        kb.button(text=f"{emoji} {asset}", callback_data=f"asset:{asset}")
    kb.button(text="🔴 Отмена", callback_data="newdeal:cancel")
...
def buyer_escrow_kb(deal_id: str) -> InlineKeyboardMarkup:
    ...
    kb.button(text="🟠 Открыть спор", callback_data=f"deal:dispute:{deal_id}")
    kb.adjust(1)
    return kb.as_markup()
...
def seller_escrow_kb(deal_id: str) -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="🔵 Вернуть деньги покупателю", callback_data=f"deal:refund:{deal_id}")
    kb.button(text="🟠 Открыть спор", callback_data=f"deal:dispute:{deal_id}")
    kb.adjust(1)
    return kb.as_markup()
...
def confirm_release_kb(deal_id: str) -> InlineKeyboardMarkup:
    ...
    kb.button(text="🔵 Назад", callback_data=f"deal:back:{deal_id}")
    kb.adjust(1)
    return kb.as_markup()
...
def dispute_resolve_kb(deal_id: str) -> InlineKeyboardMarkup:
    ...
    kb.button(text="🔴 Вернуть покупателю", callback_data=f"resolve:refund:{deal_id}")
    kb.adjust(1)
    return kb.as_markup()
